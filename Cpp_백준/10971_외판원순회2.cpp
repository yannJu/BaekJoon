...
// --- Cost 를 계산 하는 Func (Recursive) ---
void getCost(int **zero, int **ary, int t, int *cost, int c, int cnt, int r){
    int *zeroAry = *zero; // ckAry
    int ct = cnt; //마지막 차례인지 ck
    int cst = c; //min Cost를 구하기 위해 할당한 변수
    
    ct += 1; //turn 수 +1
    if (ct > t) { //마지막 turn 인 경우
        *cost = min(*cost, cst); // cost 를 최소로
        return;
    }

    for (int i = 0; i < t; i++){
        // cout << "r : " << r << " i : " << i << endl;
        // --- 자기자신이거나, 마지막 turn 이 아닌데 0번째를 접근하거나, 이미 접근한 곳은 접근하지 않음 ---
        if (ary[r][i] == 0) continue;
        if (i == 0 && ct < t) continue;
        if (zeroAry[i] == 1) continue;
        cst += ary[r][i];
        zeroAry[i] = 1;
        // cout << " c(before) : " << cst << endl;
        getCost(&zeroAry, ary, t, cost, cst, ct, i);
        cst -= ary[r][i];
        zeroAry[i] = 0;
        // cout << " c(after) : " << cst << endl;
    }
}

int main(){
    int t_case, cost = 1000000000;

    cin >> t_case;
    int **ary = new int*[t_case];
    int *zeroAry = new int[t_case];

    for (int i = 0; i < t_case; i++){
        ary[i] = new int[t_case];
        zeroAry[i] = 0;
    ...
    getCost(&zeroAry, ary, t_case, &cost, 0, 0, 0);
    cout << cost << endl;
}