...
void check(INFO *info, int **rlt, int t){
    int *result = *rlt;
    // --- 하나하나 ck ---
    for (int i = 0; i < t; i++){
        for (int j = 0; j < t; j++){
            if (i == j) continue;
            // 몸무게, 키가 더 큰경우에만 +1
            if (info[i].w < info[j].w && info[i].h < info[j].h) result[i] += 1;
        }
    }
...
int main(){
    ...
    cin >> t_case;
    info = new INFO[t_case];
    result = new int[t_case];
    ...
    check(info, &result, t_case);
    for (int i = 0; i < t_case; i++) cout << result[i] + 1 << " ";
    cout << endl; 
}