...
2 초	128 MB	86695	29451	21089	32.829%
문제
...
int main(){
    ...
    for (int i = 0; i < wines; i++){
        cin >> weight;
        Wine.push_back(weight);
    }
    for (int i = 1; i <= wines; i++){
        if (i == 1) DP.push_back(Wine[i]);
        else if (i == 2) DP.push_back(Wine[i - 1] + Wine[i]);
        else {
            tmp = max(DP[i - 3] + Wine[i - 1] + Wine[i], DP[i - 2] + Wine[i]);
            tmp = max(tmp, DP[i - 1]);
            DP.push_back(tmp);
        }
    }
