...
sys.setrecursionlimit(10**7) #Recursive 의 깊이 수정
N = int(input()) #TestCase
ckLst = [] #Ck Lst
zero = [0 for i in range(N)]
lst = list() #전체 lst
dirLst = [(0, -1), (0, 1), (-1, 0), (1, 0)] #상하좌우

cnt = 0 #영역 수
mxCnt = -1 #result cnt 값

def dfs(lst, cklst, N, d, mx, x, y, cnt):
    if (cklst[y][x] == 0 and lst[y][x] > mx):
        cklst[y][x] = 1
        #print("cnt : ", cnt, "\ncklt : ", cklst)
        for dr in d: #상하좌우 탐색
            if (x + dr[0] >= 0) and (y + dr[1] >= 0) and (x + dr[0] < N) and (y + dr[1] < N):
                cnt = min(cnt, dfs(lst, ckLst, N, d,  mx, x + dr[0], y + dr[1], cnt))
        cnt = cnt + 1
    #print("(x , y) : ({}, {})".format(x, y), "CNT : ", cnt)
    return cnt
# --- lst받기 ---
for i in range(N): 
...
for mx in range(m + 2):
    # --- ckLst Initialize ---
    ckLst = []
    for j in range(N): ckLst.append([0 for i in range(N)]) 
    cnt = 0
    # --- 탐색 ---
    for l in range(N):
        for k in range(N):
            if (ckLst[l][k] == 0): cnt = dfs(lst, ckLst, N, dirLst, mx, k, l,  cnt)
    # print("mx : ", mx, "CNT : ", cnt)
    # print("============")
