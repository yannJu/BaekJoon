...
입력
첫째 줄에 참가자의 수 N과 1 라운드에서 김지민의 번호와 임한수의 번호가 순서대로 주어진다. N은 100,000보다 작거나 같은 자연수이고, 
...
people, player_a, player_b = map(int, sys.stdin.readline().split())
...
def ckIsFight(a, b, c):
    ...
    # print("C ; ", cnt)
    if (a % 2 != 0): #a가 홀수라면
        if (a + 1 == b): return cnt #상대가 맞다면
        else:  a = a // 2 + 1 #상대가 아니라면
    else: #a가 짝수
        if (a - 1) == b: return cnt #상대가 맞다면
        else: a = a // 2 #상대가 아니라면
    
    if (b % 2 == 0): b = b // 2 #b가 짝수라면
    else: b = b // 2 + 1
    
    return ckIsFight(a, b, cnt)    
    
cnt = ckIsFight(player_a ,player_b, 0)
if (cnt == 0): cnt = -1
print(cnt)